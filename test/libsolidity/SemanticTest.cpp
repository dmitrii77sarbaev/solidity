...
bool SemanticTest::checkGasCostExpectation(TestFunctionCall& _test, bool _compileViaYul) const
{
	...
	_test.setGasCost(setting, m_gasUsed);
	return
		_test.call().expectations.gasUsed.count(setting) > 0 &&
		m_gasUsed == _test.call().expectations.gasUsed.at(setting);
}
...
{
	for (auto const& test: m_tests)
		_stream << test.format("", !m_gasCostFailure, false);
}
